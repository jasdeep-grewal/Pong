...
void CPong::draw(int x, int y)
{
	...
	*/
	...
	if ((xPos < 30) && ((yPos < (y + 50)) && (yPos > (y - 50))))
		xVel = -xVel;
		

	cv::rectangle(_canvas, cv::Point(990, 50 + yPos), cv::Point(1000, yPos - 50), cv::Scalar(255, 255, 255), CV_FILLED, 0);
	...
	cv::rectangle(_canvas, cv::Point(0, y-50), cv::Point(10, 50+y), cv::Scalar(255, 255, 255), CV_FILLED, y);
